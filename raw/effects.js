...
export function marquee(input, displayAmount = input.length, options) {
    ...
    for (let i = displayAmount - 1; i >= Math.min(0, displayAmount - input.length); i -= 1) {
        const leftPad = Math.max(0, i); // Количество пустых дисплеев слева в начале
        const inputStart = Math.max(0, -i); // Индекс символа в строке, с которого начинается показ
        const visibleLength = displayAmount - leftPad; // Максимальное количество показываемых символов
        const visiblePart = input.slice(inputStart, inputStart + visibleLength); // Aware, получившаяся строка может быть короче visibleLength
